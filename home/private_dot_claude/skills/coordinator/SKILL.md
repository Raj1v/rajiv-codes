...
- Include this instruction at the top: `Before starting, run: source .env.workmux 2>/dev/null && gh auth setup-git && pnpm install`
...
**IMPORTANT: `workmux add` calls must run sequentially (no `&`, no parallel
Bash calls). Parallel spawns cause git config lock contention and zombie
worktrees. The agents themselves still run in parallel — it's only the spawn
commands that must be sequential.**

```bash
# Step 1: Write all prompt files (parallel is fine here)
tmpfile_a=$(mktemp).md
cat > "$tmpfile_a" << 'EOF'
...
# Step 2: Spawn agents sequentially (agents run in parallel once spawned)
...
# 2. Spawn agents sequentially (they run in parallel once spawned)
